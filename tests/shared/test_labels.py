...
from sip_engine.shared.data.label_builder import (
    M1_TIPOS,
    M2_TIPOS,
...
def _make_contrato_row(proceso, id_contrato, tipo_doc, doc_num):
    ...
    return (
        ...
        f"Bogota,Servicios,2023-01-01,2023-01-10,365 Dia(s),0"
    )
...
def test_m2_sparsity_warning(tmp_path, monkeypatch, caplog):
    ...
    try:
        with caplog.at_level(logging.WARNING, logger="sip_engine.shared.data.label_builder"):
            build_labels()
    finally:
        reset_rcac_cache()
