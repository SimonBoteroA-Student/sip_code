...
class TestCalibrateThresholds:
    def test_calibrate_basic(self, simple_df):
        ...
        from sip_engine.classifiers.iric.thresholds import calibrate_iric_thresholds
...
class TestGetThreshold:
    def test_get_threshold_exact_match(self, sample_thresholds):
        ...
        from sip_engine.classifiers.iric.thresholds import get_threshold
...
class TestLoadSaveThresholds:
    ...
    def test_save_creates_parent_dir(self, sample_thresholds):
        ...
        from sip_engine.classifiers.iric.thresholds import save_iric_thresholds
...
class TestUnicoProponente:
    def test_unico_proponente_fires_on_single_bidder(self, base_row, minimal_thresholds):
        ...
        from sip_engine.classifiers.iric.calculator import compute_iric_components
...
class TestIricScores:
    def test_iric_score_formula(self, minimal_thresholds):
        ...
        from sip_engine.classifiers.iric.calculator import compute_iric_scores
...
class TestIRICProviderHistoryIntegration:
    ...
    def test_component_9_fires_with_real_schema_key(self, base_row, procesos_normal, thresholds):
        ...
        """
        from sip_engine.classifiers.features.provider_history import _ZERO_RESULT
        from sip_engine.classifiers.iric.calculator import compute_iric_components
...
class TestBuildIricCreatesParquet:
    ...
    def test_build_iric_creates_parquet(self, tmp_path, monkeypatch):
        ...
        from sip_engine.shared.config.settings import Settings
        ...
        import sip_engine.shared.config
        sip_engine.shared.config.get_settings.cache_clear()
        ...
        with patch("sip_engine.classifiers.iric.pipeline._build_iric_procesos_lookup", return_value={}), \
             patch("sip_engine.classifiers.iric.pipeline._build_iric_num_actividades_lookup", return_value={}), \
             patch("sip_engine.classifiers.iric.pipeline.build_bid_stats_lookup", return_value={}), \
             patch("sip_engine.classifiers.features.provider_history.build_provider_history_index"), \
             patch("sip_engine.classifiers.features.provider_history.lookup_provider_history", return_value=None), \
             patch("sip_engine.shared.data.loaders.load_contratos", mock_load_contratos):
...
class TestIricInitExports:
    """sip_engine.classifiers.iric __init__.py re-exports all public symbols."""

    def test_iric_init_exports_calculator_functions(self):
        """compute_iric_components and compute_iric_scores importable from sip_engine.classifiers.iric."""
    ...
    def test_iric_init_exports_bid_stats_functions(self):
        """compute_bid_stats and build_bid_stats_lookup importable from sip_engine.classifiers.iric."""
    ...
    def test_iric_init_exports_threshold_functions(self):
        """All 5 threshold functions importable from sip_engine.classifiers.iric."""
        from sip_engine.classifiers.iric import (
    ...
    def test_iric_init_exports_pipeline_functions(self):
        """build_iric and compute_iric importable from sip_engine.classifiers.iric."""
    ...
    def test_iric_init_all_symbols_count(self):
        """sip_engine.classifiers.iric.__all__ must contain all 11 exported symbols."""
        import sip_engine.classifiers.iric
        assert len(sip_engine.classifiers.iric.__all__) == 11

    def test_iric_init_all_symbols_importable(self):
        """Every symbol in __all__ must be importable from sip_engine.classifiers.iric."""
        import sip_engine.classifiers.iric
        for symbol in sip_engine.classifiers.iric.__all__:
            obj = getattr(sip_engine.classifiers.iric, symbol, None)
